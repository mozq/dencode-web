<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"
%><h3>Über Binär-Strings</h3>
<p>Binär-Strings stellen den binären Wert einer Zeichenkette als Folge von 0 und 1 dar.</p>
<p>Da der binäre Wert von der Zeichenkodierung abhängt, variiert auch das Ergebnis der Konvertierung.</p>

<p>Beispiel für die Konvertierung von „サンプル“ (Sample) in einen Binär-String:</p>

<div class="table-responsive">
	<table class="table">
		<tr><th>Zeichenkodierung</th><th>Binär-String</th></tr>
		<tr><td>UTF-8</td><td>11100011 10000010 10110101 11100011 10000011 10110011 11100011 10000011 10010111 11100011 10000011 10101011</td></tr>
		<tr><td>UTF-16</td><td>00110000 10110101 00110000 11110011 00110000 11010111 00110000 11101011</td></tr>
		<tr><td>Shift_JIS</td><td>10000011 01010100 10000011 10010011 10000011 01110110 10000011 10001011</td></tr>
	</table>
</div>
