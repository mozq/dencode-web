...
%><h3>關於維吉尼亞密碼 (Vigenere Cipher)</h3>
<p>維吉尼亞密碼是一種透過替換文本中的字元來進行加密的替換密碼。</p>
<p>字元的替換是透過加密金鑰和以下的維吉尼亞方陣進行的。</p>
...
<p>最上面的一行是加密前的明文，左側列是加密金鑰，它們交叉處的字元就是加密後的密文。加密金鑰是由「A」到「Z」組成的字串。</p>

<p>例如，加密前的明文是「MESSAGE」，加密金鑰是「SECRET」，加密如下。第一個字元是「M」，金鑰是「S」，因此加密後的字元是「E」。</p>
...
<p>明文的每個字元使用一個加密金鑰字元。如果金鑰長度短於明文，則重複使用金鑰。</p>
