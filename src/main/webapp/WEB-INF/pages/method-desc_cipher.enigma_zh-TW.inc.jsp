...
<p>恩尼格瑪密碼是一種透過替換文本中的字元來進行加密的替換密碼。支援「A」到「Z」這 26 個字元的加密。</p>
<p>字元的替換是透過恩尼格瑪密碼機進行的。DenCode 支援以下恩尼格瑪密碼機的模擬。</p>
...
<p>恩尼格瑪密碼機的構造如下。從鍵盤 (Tastatur) 輸入的字元，經過插線板 (Steckerbrett)、輸入輪 (ETW, Eintrittswalze)、3 或 4 個轉子 (Walze)、反射器 (UKW, Umkehrwalze)，然後沿相反路徑返回，加密結果顯示在燈板 (Lampenfeld) 上。在轉子等所有位置都會進行字元替換。</p>
...
<p>輸入輪、轉子和反射器內部都有將「A」到「Z」這 26 個字元轉換為其他字元的接線，通電後即可進行替換。例如，Enigma I 的轉子「I」具有以下接線，「A」被替換為「E」。如果從反射器返回的字元是「J」，則沿相反接線被替換為「Z」。</p>
...
<p>插線板允許使用者透過接線進行替換。部分恩尼格瑪密碼機在 ETW 前端有插線板。插線板有「A」到「Z」 26 個字母的輸入輸出端子，透過電纜連接任意兩個字母，可以交換這兩個字母。例如，連接「A」和「M」，則「A」換成「M」，「M」換成「A」。未連接電纜的字母不進行替換。</p>
<p>從鍵盤輸入一個字元，轉子就會旋轉一步。轉子從右側開始旋轉，當旋轉到轉子上的凹槽位置時，左側的轉子也會旋轉一步。這種旋轉改變了每個字元的加密接線，因此即使輸入相同的字元，也會被替換為不同的字元。</p>
<p>轉子上有環，環的外周刻有「A」到「Z」（或「01」到「26」）的字元。環可以分 26 級設定外周字元與轉子內部接線的偏移量。例如，Enigma I 的轉子「I」，若環設定為「A (01)」，「A」換成「E」；若環設定為「B (02)」，內部接線偏移 1 位，原來的 Z-J 連接使得「A」換成「K」。</p>
...
<p>轉子可以設定排列順序和初始旋轉位置。例如，有「I」、「II」、「III」三種轉子，可以按「II」、「I」、「III」的順序設定，每個轉子的初始位置也可以在「A (01)」到「Z (26)」之間設定。反射器也有多種可更換類型，或是可設定初始位置的類型。輸入輪通常是固定的。帶有插線板的機器還可以設定插線板。這些設定構成了恩尼格瑪密碼機的加密金鑰。</p>
...
環設定   : X M V  (24 13 22)
初始位置 : A B L  (01 02 12)
插線板   : AM FI NV PS TU WZ

加密前的文章 : SECRET
加密後的文章 : LCGODU
</pre>

<p>這種情況下，第一個字元「S」經過以下流程最終加密為「L」。</p>
...
<p>如果用環上的字元表示轉子的旋轉位置和輸入 (+)/ 輸出 (-) 位置，則如下所示。（由於轉子在旋轉，上述「被替換的字元」與「環上位置對應的字元」不同。）</p>

<pre>
           -      +       
ABCDEFGHIJKLMNOPQRSTUVWXYZ  : 插線板
           -   +          
...
<h4>插線板連線 (Steckerverbindungen)</h4>
<p>設定插線板的連線對。</p>
<p>在 DenCode 中，透過用空格分隔並列出要交換的兩個字元對來指定連線，如「AB CD EF GH IJ KL」。此例表示「A」和「B」、「C」和「D」等連線對。</p>

<h4>Uhr</h4>
<p>Uhr 是連接到插線板並在「00」到「39」的 40 種接線方式中進行選擇的配件。插線板和 Uhr 透過 20 根電纜連接。電纜兩兩成對，共有 10 對。當 Uhr 設定為「00」時，相當於這些電纜對直接在插線板上連接。</p>
<p>只有 Enigma I 可以設定 Uhr。設定 Uhr 需要先指定 10 組插線板連線對。</p>

<h4>UKW-D 連線</h4>
<p>UKW-D 是一種可更改內部接線的反射器。</p>
<p>普通反射器的環標記為「ABCDEFGHIJKLMNOPQRSTUVWXYZ」，但 UKW-D 的標記是「A-ZXWVUTSRQPON-MLKIHGFEDCB」這種特殊順序。標記上的兩個「-」（對應普通標記的 B 和 O）是固定的，始終相互連接且無法更改。您可以設定其他 24 個字元的 12 對連線。</p>
