...
%><h3>비즈네르 암호에 대해서</h3>
<p>비즈네르 암호는 문장의 문자를 다른 문자로 치환하여 암호화하는 환자 암호 중 하나입니다.</p>
<p>문자 치환은 암호화 키와 다음의 비즈네르 표(Vigenère square)에 의해 수행합니다.</p>
...
<p>최상단 행이 암호화 전의 평문이고, 좌측 열이 암호화 키, 그것들이 교차하는 문자가 암호화 후의 문자입니다. 암호화 키는 'A'부터 'Z'까지의 문자열입니다.</p>

<p>예를 들어, 암호화 전 평문이 'MESSAGE'이고 암호화 키가 'SECRET'인 경우, 다음과 같이 문자가 치환되어 암호화됩니다. 첫 문자가 'M'이고 키는 'S'이기 때문에, 암호화 후의 문자는 'E'가 됩니다.</p>
...
<p>평문의 1문자마다 암호화 키 1문자가 사용됩니다. 키의 길이가 평문보다 짧은 경우는 키를 반복해서 사용합니다.</p>
